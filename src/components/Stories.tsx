import { useEffect, useState } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { motion } from 'framer-motion';
import { ArrowRightIcon, CalendarIcon } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { getAllStories, type Story } from '../data/supabaseStories';

export function Stories() {
  const { t, language } = useLanguage();
  const location = useLocation();
  const [stories, setStories] = useState<Story[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchStories = async () => {
      try {
        setLoading(true);
        const lang = location.pathname.startsWith('/en') ? 'en' : 'es';
        const data = await getAllStories(lang);
        setStories(data.slice(0, 3));
      } catch (err) {
        console.error('Error fetching stories:', err);
      } finally {
        setLoading(false);
      }
    };
    fetchStories();
  }, [location.pathname]);

  const getLangPrefix = () => {
    const path = location.pathname;
    if (path.startsWith('/en')) return '/en';
    if (path.startsWith('/es')) return '/es';
    return '/es';
  };

  const langPrefix = getLangPrefix();

  return (
    <section className="py-32 bg-[#080e1a] border-t border-[#1a2840]/50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-end justify-between gap-6 mb-12">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}>
            <h2 className="font-display text-3xl md:text-4xl font-light text-white mb-4 tracking-tight">{t.stories.title}</h2>
            <p className="text-content-primary max-w-xl">{t.stories.subtitle}</p>
          </motion.div>

          <motion.div
            initial={{ opacity: 0, x: 20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}>
            <Link
              to={`${langPrefix}/stories`}
              className="group inline-flex items-center gap-2 text-[#5A8BE0] hover:text-white font-medium transition-colors">
              {t.stories.viewAll}
              <ArrowRightIcon className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </Link>
          </motion.div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {loading
            ? Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="h-full bg-[#0d1424] border border-[#1a2840] rounded-xl p-8 animate-pulse flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <div className="h-5 w-20 rounded-full bg-[#1a2840]" />
                      <div className="h-4 w-20 rounded bg-[#1a2840]" />
                    </div>
                    <div className="space-y-3 mb-6">
                      <div className="h-5 w-3/4 rounded bg-[#1a2840]" />
                      <div className="h-4 w-full rounded bg-[#111c30]" />
                      <div className="h-4 w-5/6 rounded bg-[#111c30]" />
                    </div>
                    <div className="flex items-center gap-3 mb-6">
                      <div className="w-8 h-8 rounded-full bg-[#1a2840]" />
                      <div className="space-y-2 flex-1">
                        <div className="h-3 w-24 rounded bg-[#1a2840]" />
                        <div className="h-3 w-20 rounded bg-[#111c30]" />
                      </div>
                    </div>
                  </div>
                  <div className="pt-6 border-t border-[#1a2840] mt-auto">
                    <div className="h-4 w-24 rounded bg-[#1a2840]" />
                  </div>
                </div>
              ))
            : stories.map((story, index) => (
                <motion.article
                  key={story.slug}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}>
                  <Link
                    to={`${langPrefix}/stories/${story.slug}`}
                    className="group flex flex-col h-full bg-[#0d1424] border border-[#1a2840] rounded-xl overflow-hidden hover:border-[#1D49A7]/40 transition-colors duration-300">
                    <div className="p-8 flex flex-col flex-grow">
                      <div className="flex items-center justify-between mb-6">
                        <span className="text-xs font-medium text-[#5A8BE0] bg-[#1D49A7]/10 px-2 py-1 rounded">
                          {story.category}
                        </span>
                        <div className="flex items-center gap-2 text-xs text-content-primary">
                          <CalendarIcon className="w-3 h-3" />
                          {story.date}
                        </div>
                      </div>

                      <h3 className="text-xl font-medium text-white mb-3 group-hover:text-[#5A8BE0] transition-colors">
                        {story.title}
                      </h3>
                      <p className="text-content-primary text-sm leading-relaxed mb-6 flex-grow">{story.excerpt}</p>

                      <div className="flex items-center gap-3 mb-6">
                        <img src={story.author.avatar} alt={story.author.name} className="w-8 h-8 rounded-full object-cover" />
                        <div>
                          <p className="text-sm text-white font-medium">{story.author.name}</p>
                          <p className="text-xs text-content-primary">{story.author.role}</p>
                        </div>
                      </div>

                      <div className="pt-6 border-t border-[#1a2840] mt-auto">
                        <span className="text-sm text-white font-medium group-hover:underline decoration-[#5A8BE0] underline-offset-4">
                          {t.stories.readStory}
                        </span>
                      </div>
                    </div>
                  </Link>
                </motion.article>
              ))}
        </div>
      </div>
    </section>
  );
}
